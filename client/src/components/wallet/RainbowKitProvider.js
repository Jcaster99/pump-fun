	...
	return (
		<WagmiProvider config={config}>
			<QueryClientProvider client={queryClient}>
				...
				>
					<RainbowProviderInner theme={theme}>{children}</RainbowProviderInner>
				</RainbowKitProvider>
			</QueryClientProvider>
		</WagmiProvider>
	)
...
export const CustomConnectButton = () => {
	...
	return (
		<ConnectButtonInner showCustomModal={showCustomModal} setShowCustomModal={setShowCustomModal} />
	)
}
							...
							{(() => {
								...
								if (chain.unsupported) {
									return (
										...
										>
											Please switch to 0G-Galileo-Testnet
										</button>
									)
								}

								return (
									<button onClick={handleAccountClick} type="button" style={connectedButtonStyle}>
										<img
											src="/wallet-avatar.png"
											alt="Profile"
											style={{ width: '32px', height: '32px', borderRadius: '50%' }}
										/>

										<span style={{ marginRight: '10px' }}>{account.displayName}</span>
									</button>
								)
							})()}
						</div>
