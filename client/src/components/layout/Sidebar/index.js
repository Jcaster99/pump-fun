...
const Sidebar = ({ isOpen, isMobile, onClose }) => {
	...
	const [selectedMode, setSelectedMode] = useState(darkMode) //true

	const handleModeChange = (mode) => {
		setSelectedMode((prev) => prev !== mode ? !prev : prev)
		...
		<div style={sidebarStyle}>
			...
			<div className="switcher-container">
				...
				>
					...
					/>
					<button
						className={`${darkMode ? 'themeDarkButton' : 'themeLightButton'} ${selectedMode ? 'active' : ''}`}
						onClick={() => handleModeChange(true)}
					>
						<span style={{ fontSize: '14px' }}>DEGEN</span>
					</button>
					<button
						className={`${darkMode ? 'themeDarkButton' : 'themeLightButton'} ${!selectedMode ? 'active' : ''}`}
						onClick={() => handleModeChange(false)}
					>
						<span style={{ fontSize: '14px' }}>EXPERT</span>
					</button>
				</div>
			</div>
		</div>
